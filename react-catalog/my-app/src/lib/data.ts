...
export function readItems(): Promise<Product[]> {
...
export function readItem(productId: number): Promise<Product | undefined> {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      Math.random() >= 0.05
        ? resolve(products.find((i) => i.productId === productId))
        : reject(new Error('What bad luck!'));
    }, 100);
  });
}
